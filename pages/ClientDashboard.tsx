...
import { Profile, Order, OrderStatus, CartItem, Business, Location, Product, UserRole, Notification, QuickMessage, BusinessLoad } from '../types';
...
import { MapPin, Bike, Clock, CheckCircle, Search, Frown, ShoppingBag, Check, ClipboardList, MessageSquare, ThumbsUp, UtensilsCrossed, PackageCheck, XCircle, Package } from 'lucide-react';
...
const fetchNearbyBusinesses = async (location: Location): Promise<Business[]> => {
    return new Promise(resolve => {
      setTimeout(() => {
        resolve([
          { id: 'b1', name: 'Taquería El Pastor', category: 'Mexicana', rating: 4.8, rating_count: 152, delivery_time: '25-35 min', delivery_fee: 30, image: 'https://picsum.photos/seed/tacos/400/300', location: { lat: 19.4300, lng: -99.1300 }, is_open: true, phone: '55-1234-5678', address: 'Calle Falsa 123, Colonia Centro, CDMX', email: 'contacto@elpastor.com', opening_hours: 'L-D: 12:00 - 23:00', products: MOCK_POPULAR_PRODUCTS.filter(p=>p.business_id === 'b1'), current_load: BusinessLoad.NORMAL },
          { id: 'b2', name: 'Sushi Express', category: 'Japonesa', rating: 4.6, rating_count: 88, delivery_time: '30-40 min', delivery_fee: 0, image: 'https://picsum.photos/seed/sushi/400/300', location: { lat: 19.4350, lng: -99.1400 }, is_open: true, phone: '55-8765-4321', address: 'Avenida Siempre Viva 742, Roma Norte, CDMX', email: 'hola@sushiexpress.com', opening_hours: 'M-S: 13:00 - 22:00', products: MOCK_POPULAR_PRODUCTS.filter(p=>p.business_id === 'b2'), current_load: BusinessLoad.BUSY },
          { id: 'b3', name: 'Pizza Bella', category: 'Italiana', rating: 4.9, rating_count: 213, delivery_time: '20-30 min', delivery_fee: 25, image: 'https://picsum.photos/seed/pizza/400/300', location: { lat: 19.4290, lng: -99.1350 }, is_open: false, phone: '55-5555-5555', address: 'Plaza Central 1, Condesa, CDMX', email: 'info@pizzabella.com', opening_hours: 'L-V: 11:00 - 21:00', products: MOCK_POPULAR_PRODUCTS.filter(p=>p.business_id === 'b3'), current_load: BusinessLoad.NORMAL },
          { id: 'b4', name: 'Burger Joint', category: 'Americana', rating: 4.5, rating_count: 340, delivery_time: '35-45 min', delivery_fee: 40, image: 'https://picsum.photos/seed/burger/400/300', location: { lat: 19.4380, lng: -99.1310 }, is_open: true, phone: '55-1122-3344', address: 'Boulevard del Sabor 55, Polanco, CDMX', email: 'burgers@joint.com', opening_hours: 'L-D: 10:00 - 22:30', products: MOCK_POPULAR_PRODUCTS.filter(p=>p.business_id === 'b4'), current_load: BusinessLoad.VERY_BUSY },
        ]);
      }, 500);
    });
};
...
const ClientDashboard: React.FC<ClientDashboardProps> = ({ user, onLogout }) => {
    ...
    const handlePlaceOrder = async (details: { location: Location; notes: string }) => {
